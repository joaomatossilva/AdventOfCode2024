...
bool CheckOperation(Operation operation1)
{
    ...
    return CheckOperationDeep(operation1, 1, '+', result) || CheckOperationDeep(operation1, 1, '*', result) || CheckOperationDeep(operation1, 1, '|', result);
}
...
bool CheckOperationDeep(Operation operation1, int index1, char op, long acc)
{
    switch (op)
    {
        ...
        case '*':
            ...
            break;
        case '|':
            acc *= (long)Math.Pow(10, GetPowerOfTen(operation1.operands[index1]));
            acc += operation1.operands[index1];
            break;
        default:
    ...
    return CheckOperationDeep(operation1, index1 + 1, '+', acc) || CheckOperationDeep(operation1, index1 + 1, '*', acc) || CheckOperationDeep(operation1, index1 + 1, '|', acc);
}

int GetPowerOfTen(int value)
{
    for (int i = 0; i < 10; i++)
    {
    ...
    throw new Exception($"No Pow: {value}");
}

...
var part2Solution = FindSolution(instructions.Length -1 );

long FindSolution(int level, long a = 0)
{
    if (level == -1)
    {
        return a;
    ...
    var instruction = instructions[level];
    a *= 8;
    for (int i = 0; i < 8; i++)
    {
        registers['A'] = a + i;
        ...
        if (output.First() == instruction)
        {
            var partial =  FindSolution(level - 1, a + i);
            if (partial == -1)
            {
                continue;
            }
